...
public class StockController {
     ...
     * Get historical data
     */
    @GetMapping("/{symbol}/historical/{timeframe}")
    public ResponseEntity<?> getHistoricalData(@PathVariable String symbol, @PathVariable String timeframe) {
        try {
            logger.info("Fetching historical data for symbol: {} with timeframe: {}", symbol, timeframe);
            return ResponseEntity.ok(stockService.getHistoricalData(symbol, timeframe));
        } catch (Exception e) {
